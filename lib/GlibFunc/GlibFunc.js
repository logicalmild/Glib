...
// Section1: Query Data
    ...
    function IncludeComponent(ElementID,Component){
        ...
        var response;
            $.ajax({ type: "GET",   
                url: Site.Url + "/SitePages/web/component/" + Component + "/"+ Component+".html",   
                async: false,
                success : function(text)
                {
                    response = text;
    ...
    function GetItemByRestAPI(Listname,Query){ 
        ...
        var requestUri = Site.Url + "/_api/web/lists/getByTitle('"+Listname+"')/items" + Query;
        var requestHeaders = {
        "accept": "application/json;odata=verbose"
        }
        var extr_Data;
...
function processSendEmails() {
    ...
    var from = 'saranchai@spoffcial.onmicrosoft.com',
        to = 'ebiz_niit1@scg.com',
